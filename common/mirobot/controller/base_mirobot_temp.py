...
class BaseMirobot(AbstractContextManager):
    ...
    def update_status(self, disable_debug=False):
        ...
        """
        # if wait = False, get only the status message and not 'ok' or 'Info,'
        while True:
            status_msg = self.get_status(disable_debug=disable_debug)[0]
            if status_msg and status_msg != 'ok' and not status_msg.startswith('Info,'):
    ...
    def _parse_status(self, msg):
        ...
        if regex_match:
            ...
            return_status = MirobotStatus("Error",
                                          self.status.angle,
                                          self.status.cartesian,
                                          self.status.pump_pwm,
                                          self.status.valve_pwm,
                                          self.status.motion_mode)
            return return_status
        else:
