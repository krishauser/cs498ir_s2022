...
#CAMERA_JITTER_TRANSLATION = 0.05
#CAMERA_JITTER_ROTATION = 0.1
CAMERA_JITTER_TRANSLATION = 0
CAMERA_JITTER_ROTATION = 0

template_folder = 'template_worlds'
#base_files = ['table.xml','box.xml','shelf.xml']
base_files = ['table.xml','box.xml']
base_boxes = []
...
def gen_grasp_worlds(N=10):
    if len(base_boxes)==0:
        ...
        for basefn in base_files:
            world = WorldModel()
            world.readFile(os.path.join(template_folder,basefn))
    ...
    num_objects = [3,3,5,5,8,8]
    for i in range(N):
        nobj = random.choice(num_objects)
        world = WorldModel()
        base_world = random.choice(range(len(base_files)))
        world.readFile(os.path.join(template_folder,base_files[base_world]))
        obj_fns = []
        for o in range(nobj):
...
def edit_camera_xform(world_fn,xform=None,title=None):
    ...
    world.readFile(os.path.join(template_folder,"camera.rob"))
    robot = world.robot(0)
...
def edit_camera_viewpoints():
    ...
    for base in base_files:
        ...
        for i,xform in enumerate(camera_viewpoints[base]):
            ...
            sensor_xform = edit_camera_xform(os.path.join(template_folder,base),xform,title="Camera transform {} {}".format(base,i))
...
def gen_grasp_images(max_variations=10):
    ...
    if len(camera_viewpoints) == 0:
        ...
        for base in base_files:
            ...
            while True:
                ...
                index += 1
            if len(camera_viewpoints[base]) == 0:
                print("Need to edit camera viewpoints for",base)
                raise RuntimeError()
    ...
    for fn in glob.glob("generated_worlds/world_*.xml"):
        ...
        def loop_through_sensors(world=world,sensor=sensor,
            ...
            #TODO: problem 1.B: perturb camera
            if CAMERA_JITTER_TRANSLATION > 0 or CAMERA_JITTER_ROTATION > 0:
                a = CAMERA_JITTER_TRANSLATION
                b = CAMERA_JITTER_ROTATION
                tcenter_local = [0,0,0.5]
                Rperturb = so3.from_rpy((random.uniform(-b,b),random.uniform(-b,b),random.uniform(-b,b)))
                tcenter = se3.apply(sensor_xform,tcenter_local)
                Rnew = so3.mul(sensor_xform[0],Rperturb)
                tnew = vectorops.sub(tcenter,so3.apply(Rnew,tcenter_local))
                tnew = vectorops.add(tnew,[random.uniform(-a,a),random.uniform(-a,a),random.uniform(-a,a)])
                sensor_xform = (Rnew,tnew)
            sensing.set_sensor_xform(sensor,sensor_xform)
            
            for i in range(world.numRigidObjects()):
                world.rigidObject(i).appearance().setColor(random.random(),random.random(),random.random())
            for i in range(world.numTerrains()):
                world.terrain(i).appearance().setColor(random.random(),random.random(),random.random())
...
def make_grasp_map(world,id):
    ...
    if gripper_robot is None:
        ...
        fn = os.path.abspath(os.path.join("../data/gripperinfo/",source_gripper.klamptModel))
        if not gripper_world.readFile(fn):
            raise ValueError("Unable to read gripper file "+fn)
        gripper_robot = gripper_world.robot(0)
    gripper_geom_open = source_gripper.getGeometry(gripper_robot,source_gripper.openConfig)
    ...
    for i in range(world.numRigidObjects()):
        ...
        gripper_tip = vectorops.madd(source_gripper.center,source_gripper.primaryAxis,source_gripper.fingerLength-source_gripper.fingerWidth/2)
        num_grasps_valid = 0
        for gindex,g in enumerate(grasps):
            #TODO: problem 1B: do collision testing
            ...
            if (id,ycb_obj,gindex) in grasps_feasible:
                ...
                gripper_geom = source_gripper.getGeometry(gripper_robot,g.finger_config)
                for approach_samples in range(50):
                    local_pt,world_pt = g.ik_constraint.getPosition()
                    local_axis,world_axis = g.ik_constraint.getRotationAxis()
                    R0 = so3.align(local_axis,world_axis)
                    Rsample = so3.rotation(world_axis,random.uniform(0,math.pi*2))
                    Rsample = so3.mul(Rsample,R0)
                    Tsample = (Rsample,vectorops.sub(world_pt,so3.apply(Rsample,local_pt)))
                    Ttest = se3.mul(obj.getTransform(),Tsample)
                    gripper_geom_open.setCurrentTransform(*Ttest)
                    gripper_geom.setCurrentTransform(*Ttest)
                    feasible = True
                    for j in range(world.numRigidObjects()):
                        if world.rigidObject(j).geometry().collides(gripper_geom_open):
                            #vis.debug("Colliding object",world.rigidObject(j),gripper_geom_open)
                            feasible=False
                            break
                        if j!=i and world.rigidObject(j).geometry().collides(gripper_geom):
                            #vis.debug("Colliding object",world.rigidObject(j).geometry(),"target",world.rigidObject(i).geometry(),gripper_geom)
                            feasible=False
                            break
                    for j in range(world.numTerrains()):
                        if not feasible:
                            break
                        if world.terrain(j).geometry().collides(gripper_geom_open):
                            #vis.debug(world.terrain(j),gripper_geom_open)
                            feasible=False
                            break
                        if world.terrain(j).geometry().collides(gripper_geom):
                            #vis.debug(world.terrain(j),gripper_geom)
                            feasible=False
                            break
                    if feasible:
                        found_approach = True
            ...
            if projected is not None:
                ...
                if x < 0 or x >= w or y < 0 or y >= h: 
                    continue
                gripper_opening_axis = so3.apply(Tworld[0],source_gripper.secondaryAxis)
                ...
                for u in range(-window,window+1):
                    ...
                    for v in range(-window,window+1):
                        ...
                        if score > result[y+u,x+v,0]:
                            result[y+u,x+v,:] = [score,source_gripper.configToOpening(g.finger_config),
...
if __name__ == '__main__':
    ...
    if PROBLEM=='worlds':
        gen_grasp_worlds()
    elif PROBLEM == 'cameras':
        edit_camera_viewpoints()
    elif PROBLEM == 'images':
        gen_grasp_images()
    else:
